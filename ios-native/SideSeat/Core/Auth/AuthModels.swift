...
enum AuthIssueMapper {
    static func message(for error: Error) -> String {
        if let api = error as? APIClientError {
            ...
            case .transport:
                return AppLocalization.string( "The network connection failed. Please try again.")
            case .decoding, .invalidResponse:
                return AppLocalization.string( "Something went wrong. Please try again.")
            }
        }
    ...
    private static func friendlyMessage(code: String, fallback: String) -> String {
        ...
        case "INVALID_CREDENTIALS":
            return AppLocalization.string( "Incorrect username or password.")
        case "USERNAME_TAKEN", "SIGNUP_USERNAME_TAKEN":
            return AppLocalization.string( "That username is already taken.")
        case "CODE_INVALID", "OTP_INVALID", "PASSWORD_RESET_CODE_INVALID":
            return AppLocalization.string( "Invalid or expired verification code.")
        case "RATE_LIMITED", "TOO_MANY_REQUESTS", "EMAIL_OTP_RATE_LIMITED":
            return AppLocalization.string( "Too many attempts. Please wait a moment and try again.")
        case "DB_UNAVAILABLE", "DB_SCHEMA",
            ...
            return AppLocalization.string( "The service is temporarily unavailable. Please try again shortly.")
        case "EMAIL_OTP_EMAIL_NOT_CONFIGURED", "EMAIL_OTP_EMAIL_SEND_FAILED":
            return AppLocalization.string( "We couldn't send the email right now. Please try again shortly.")
        default:
            return fallback
        }
    }
...
enum AuthFieldValidation {
    ...
    static func usernameIssue(_ raw: String) -> String? {
        ...
        if value.count < 2 {
            return AppLocalization.string( "Username must be at least 2 characters.")
        }
        if value.count > 32 {
            return AppLocalization.string( "Username must be at most 32 characters.")
        }
        guard value.range(of: "^[a-z0-9_-]+$", options: .regularExpression) != nil else {
            return AppLocalization.string( "Use only letters, numbers, underscores, and hyphens.")
        }
        if value.hasPrefix("guest_") {
            return AppLocalization.string( "This username is reserved.")
        }
        return nil
    }
    ...
    static func passwordIssue(_ raw: String) -> String? {
        if raw.count < 8 {
            return AppLocalization.string( "Password must be at least 8 characters.")
        }
        return nil
    }
    ...
    static func emailIssue(_ raw: String) -> String? {
        ...
        if value.count < 3 {
            return AppLocalization.string( "Enter your email.")
        }
        guard value.range(of: "^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$", options: .regularExpression) != nil else {
            return AppLocalization.string( "Enter a valid email address.")
        }
        return nil
    }
    ...
    static func otpIssue(_ raw: String) -> String? {
        ...
        guard value.range(of: "^\\d{6}$", options: .regularExpression) != nil else {
            return AppLocalization.string( "Enter the 6-digit verification code.")
        }
        return nil
    }
}
