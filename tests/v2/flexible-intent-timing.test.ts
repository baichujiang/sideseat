...
import { compatibleIntentTiming, flexiblePreferenceFitsLifecycle, type IntentTimePreference } from "../../lib/v2/intent-timing";
...
test("undecided is valid input with no availability and does not expire", () => {
  ...
  assert.equal(weeklyIntentPatchSchema.safeParse({ action: "EXTEND", expectedVersion: 1 }).success, false);
  assert.equal(compatibleIntentTiming(undecided, undecided, new Date("2026-10-09T10:00:00Z"))?.expiresAt.toISOString(), "2026-10-11T10:00:00.000Z");
});
