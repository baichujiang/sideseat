...
import { canCancelActivity } from "@/lib/discover/discover-activity-state";
import {
...
export async function POST(
  ...
  if (!canCancelActivity(viewer, activity, new Date())) {
    return error(
  ...
  const updated = await prisma.discoverActivity.update({
    where: { id },
    data: { status: DiscoverActivityStatus.CANCELED },
