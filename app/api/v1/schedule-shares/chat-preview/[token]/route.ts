...
export async function GET(
  ...
  try {
    const preview = await getScheduleShareChatPreview({
      userId: auth.user.id,
      token: decoded,
    });
    return v1Success(preview, { request });
  } catch (cause) {
    if (cause instanceof ScheduleShareServiceError) {
      return v1Error(request, mapScheduleShareError(cause));
    }
    console.error("GET /api/v1/schedule-shares/chat-preview/[token]", cause);
    return v1Error(request, {
      code: "INTERNAL_ERROR",
