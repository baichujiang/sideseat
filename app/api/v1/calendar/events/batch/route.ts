...
import {
  createCalendarEventsBatchForUser,
  normalizeCalendarEventsBatch,
} from "@/lib/calendar/create-calendar-events-batch";
...
export async function POST(request: Request) {
  ...
  const parsed = await parseV1Json(request, batchCalendarEventsSchema);
  if (!parsed.ok) return parsed.response;
  const values = { events: normalizeCalendarEventsBatch(parsed.data.events) };

  try {
    const rateLimit = await consumeV1RateLimit({
      scope: "native-calendar-batch-write",
    ...
    if (!rateLimit.allowed) {
      return v1Error(request, {
        code: "RATE_LIMITED",
        message: "Too many calendar batches were created. Try again shortly.",
        status: 429,
        retryable: true,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const claim = await claimIdempotency(tx, {
        scope: "native-calendar-event-batch-create",
      ...
      const body = await createCalendarEventsBatchForUser(auth.user, values.events, tx);
    ...
    console.error("POST /api/v1/calendar/events/batch", cause);
    return v1Error(request, {
      code: "INTERNAL_ERROR",
      message: "The calendar events could not be created.",
      status: 500,
      retryable: true,
    });
  }
