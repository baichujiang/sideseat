...
export async function POST(
  ...
  try {
    return await runIdempotentV1Mutation({
      ...
      scope: `native-friend-link:${connectionId}`,
      requestBody: parsed.data,
      execute: async () => ({
        status: 200,
        body: await mutateFriendLink({
          userId: auth.user.id,
          connectionId,
    ...
    console.error("POST /api/v1/connections/[connectionId]/friend-link", cause);
    return v1Error(request, {
      code: "INTERNAL_ERROR",
      message: "The friend link could not be updated.",
      status: 500,
      retryable: true,
    });
  }
