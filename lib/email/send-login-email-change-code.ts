...
function buildChangeCodeHtml({ code }: { code: string }) {
  return `
    <div style="margin:0;padding:32px 16px;background:#f4efe6;font-family:Arial,Helvetica,sans-serif;color:#23313d;">
      <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e6ddd0;border-radius:24px;padding:32px 24px;">
        ...
        <h1 style="margin:0 0 16px;font-size:28px;line-height:1.2;color:#17222b;">Confirm your new login email</h1>
        <p style="margin:0 0 24px;font-size:15px;line-height:1.7;color:#44515b;">
          Enter this code in the app to update the email you use to sign in. It expires in 10 minutes.
        </p>
...
export type SendLoginEmailChangeCodeResult =
  | { sent: true; id: string | null }
  | { sent: false; reason: string };

export async function sendLoginEmailChangeCodeEmail({
  email,
  code,
}: {
  email: string;
  code: string;
}): Promise<SendLoginEmailChangeCodeResult> {
  ...
  try {
    const result = await resend.emails.send({
      from,
      to: email,
      subject: "Your SideSeat login email change code",
      html: buildChangeCodeHtml({ code }),
    });
