  ...
  return (
    <div className="space-y-3">
      <p className="text-[11px] leading-snug text-muted-foreground">
        ...
        Tap a colored block to edit. Mon–Fri only on the grid — weekend below.
      </p>
          ...
          <div className="flex min-w-0 flex-1 items-stretch">
            {WORKDAYS.map((weekday) => (
              ...
              >
                ...
                <div className="relative shrink-0" style={{ height: bodyHeight }}>
                  ...
                  if (session.weekday !== weekday) return null;
                  const b = sessionBounds(session);
                  if (!b) return null;
                  const visStart = Math.max(b.start, GRID_VIEW_START);
                  const visEnd = Math.min(b.end, GRID_VIEW_END);
                  ...
                  return (
                    <button
                      key={`${index}-${session.start}-${session.end}`}
                      type="button"
                      className={cn(
                        "absolute left-0.5 right-0.5 z-10 overflow-hidden rounded-md border px-1 py-0.5 text-left shadow-sm transition",
