...
export function ProfileNameEditor({
  ...
  const [draft, setDraft] = useState(initialNickname.trim());
  const [error, setError] = useState("");
  const [pending, startTransition] = useTransition();

  const loginHint = loginUsername?.trim()
    ? t.loginUsernameNameSheetHint
    : null;
  ...
  return (
    <div className="space-y-4">
      ...
      {loginHint ? <p className="text-[12px] leading-snug text-muted-foreground">{loginHint}</p> : null}
