...
1. **Physical device** — re-run `capture-visual-qa.sh` on a paired iPhone when available (Simulator only today).
...
# optional: SIM_DEVICE_ID=<udid> ios-native/scripts/capture-visual-qa.sh
```
